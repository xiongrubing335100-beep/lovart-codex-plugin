...
class AgentSkill:
    ...
    @staticmethod
    def artifact_urls(result: Optional[dict]) -> set:
        """Return the distinct artifact URLs present in a result."""
        return {
    ...
    @staticmethod
    def download_artifacts(result: dict, output_dir: str = "/tmp/lovart", prefix: str = "lovart",
                           exclude_urls=None) -> list:
    ...
    @staticmethod
    def _result_signature(result: Optional[dict]) -> str:
        comparable = {
            "items": (result or {}).get("items", []),
            "pending_confirmation": (result or {}).get("pending_confirmation"),
        }
        return json.dumps(comparable, sort_keys=True, ensure_ascii=False)

    def poll(self, thread_id: str, timeout: Optional[int] = None, verbose: bool = False,
             baseline_result: Optional[dict] = None) -> str:
        ...
        poll_count = 0
        baseline_signature = (
            self._result_signature(baseline_result)
        ...
        while time.time() < deadline:
            ...
            if s["status"] == "done":
                if baseline_signature is not None:
                    try:
                        current_result = self.get_result(thread_id)
                        pc = current_result.get("pending_confirmation")
                        if pc:
                            self._pending_confirmation = pc
                            return "pending_confirmation"
                        if self._result_signature(current_result) == baseline_signature:
                ...
                if s2["status"] in ("done", "abort"):
                    ...
                    try:
                        ...
                        if pc:
                            ...
                            return "pending_confirmation"
                        if (baseline_signature is not None
                                and self._result_signature(result) == baseline_signature):
                            time.sleep(self.poll_interval)
                            continue
                    except Exception:
                        pass
    ...
    def chat(self, prompt: str, project_id: Optional[str] = None,
        ...
        self._previous_artifact_urls = set()
        if thread_id:
            try:
                baseline_result = self.get_result(thread_id)
                self._previous_artifact_urls = self.artifact_urls(baseline_result)
            except Exception:
                pass
        if not project_id:
        ...
        tid = self.send(prompt=prompt, project_id=project_id,
                        ...
                        mode=mode)
        status = self.poll(tid, timeout=timeout, baseline_result=baseline_result)
...
class LocalState:
    ...
    @staticmethod
    def _decode_state_bytes(raw: bytes) -> dict:
        try:
    ...
    def load(self) -> dict:
        ...
        try:
            with open(self.path, "rb") as f:
                data = self._decode_state_bytes(f.read())
...
def main():
    ...
    try:
        if args.command == "chat":
            ...
            if args.download:
                dl = skill.download_artifacts(
                    result,
                    output_dir=args.output_dir,
                    exclude_urls=getattr(skill, "_previous_artifact_urls", set()),
                )
