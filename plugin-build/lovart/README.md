...
On Windows, ask the plugin to “更换密钥” or run `scripts/configure-lovart-credentials.ps1` for a password-style setup window. The MCP reads the latest saved user-level values before every Lovart call, so replacing a key does not require restarting Codex.
